...
def train_classifier(df: DataFrame,
                     target_col_name: str,
    ...
    cl_type = pcc.oop.get_classification_task(current_experiment.y)
    modelstar_record(record_type='md',
                     content=f'**ML Type:     ** {cl_type} Classification')
    ...
    try:
        ...
        modelstar_record(
            record_type='md', content='## Which features are important for performing **classification** on the dataset?')
    ...
    modelstar_register_pycaret_inference_udf(
        function_name='predict_classifier', model_filename='classifier_model.joblib.gz', handler_args=handler_args_list)

    return {'status': 'Model training success', 'inference_function': 'predict_classifier'}
